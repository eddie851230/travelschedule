...
const AddSpot = (props) => {
    ...
    const handleDeleteSpot = (id) => {
        ...
        http.delete(`/favorite/spot/${id}`)
            .then(() => props.updateSpotinfo(props.spotinfo.filter(r => r.id !== id)))
    ...
    const handleDeleteHotel = (id) => {
        ...
        http.delete(`/favorite/hotel/${id}`)
            .then(() => props.updateHotelinfo(props.hotelinfo.filter(r => r.id !== id)))
            .then(() => props.setTrigger(false))
            .catch(error => console.log(error));
    ...
    return (props.trigger) ? (
        ...
        <div className='AddSpot'>
            <div className='favlist'>
                ...
                <div className='muti'>
                    <button
                        onClick={props.spotinfo ? () => addSchedule(props.detail.attraction_id) : updatehotel(props.detail.room_id)}
                    >加入行程表</button>
                    <button
                        style={{ 'width': '50px', 'color': '#fff', 'backgroundColor': 'red' }}
                        onClick={props.spotinfo ? () => handleDeleteSpot(props.detail.id) : () => handleDeleteHotel(props.detail.id)}
                    >刪除</button>
                </div>
                <div className='closebtn' onClick={() => props.setTrigger(false)}><div>&times;</div></div>
            </div >
